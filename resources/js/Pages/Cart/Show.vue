...
const updateQuantity = (item, quantity) => {
    router.put(route('cart.update', { item: item.id }), {
        quantity: quantity
    }, {
        preserveScroll: true
    });
...
const removeItem = (item) => {
    if (confirm('Êtes-vous sûr de vouloir retirer ce produit du panier ?')) {
        router.delete(route('cart.remove', { item: item.id }), {
            preserveScroll: true
        });
    }
};
    ...
    <AppLayout>
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div v-if="cart.items && cart.items.length > 0" class="bg-white rounded-lg shadow-md">
                    <!-- Items du panier -->
                    <div class="divide-y divide-gray-200">
                        <div v-for="item in cart.items" :key="item.id" class="p-6">
                            <div class="flex items-center space-x-4">
                                <img :src="item.product.cover_image" alt="" class="w-20 h-20 object-cover rounded">
                                <div class="flex-1">
                                    <h3 class="text-lg font-semibold">{{ item.product.label }}</h3>
